...
# setup.sh — Universal setup script for i-human
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/i-human/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
main() {
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/i-human.git"
        info "Then: cd i-human && ./setup.sh"
        exit 1
    fi
