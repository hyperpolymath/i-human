...
// Replace i-human, i-human, {{DEPS}} with actuals
= i-human — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining i-human for
...
| `$PREFIX/share/i-human/`
| Data files, assets

| `$PREFIX/share/doc/i-human/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/i-human/config.toml`

Fallback: `$HOME/.config/i-human/config.toml`
...
Or via OPSM: `opsm update i-human`
...
[source,bash]
----
just install --prefix=/opt/i-human-instance1 --config=/etc/i-human/instance1.toml
just install --prefix=/opt/i-human-instance2 --config=/etc/i-human/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/i-human/issues
