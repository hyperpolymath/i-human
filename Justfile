...
init:
    ...
    # Also replace i-human and hyperpolymath in AI manifest
...
cookbook:
    ...
    echo "= i_human Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/i_human.1 << EOF
    .TH i_human 1 "$(date +%Y-%m-%d)" "{{version}}" "i_human Manual"
    .SH NAME
    i_human \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/i_human.1"
...
container-build tag="latest":
    @if [ -f Containerfile ]; then \
        podman build -t i_human:{{tag}} -f Containerfile .; \
    else \
...
# Run container
container-run *args:
    podman run --rm -it i_human:latest {{args}}

# Push container image
container-push registry="ghcr.io/hyperpolymath" tag="latest":
    podman tag i_human:{{tag}} {{registry}}/i_human:{{tag}}
    podman push {{registry}}/i_human:{{tag}}
...
doctor:
    @echo "Running diagnostics for i-human..."
...
heal:
    @echo "Attempting auto-repair for i-human..."
...
help-me:
    ...
    @echo "  https://github.com/hyperpolymath/i-human/issues/new"
    @echo ""
