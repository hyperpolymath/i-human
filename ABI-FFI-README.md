...
│       └── include/
│           └── i_human.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── i_human.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/i_human.h
```
...
int main() {
    void* handle = i_human_init();
    if (!handle) return 1;

    int result = i_human_process(handle, 42);
    if (result != 0) {
        const char* err = i_human_last_error();
...
Compile with:
```bash
gcc -o example example.c -li_human -L./zig-out/lib
```

### From Idris2

```idris
import I_HUMAN.ABI.Foreign
...
#[link(name = "i_human")]
extern "C" {
    fn i_human_init() -> *mut std::ffi::c_void;
    fn i_human_free(handle: *mut std::ffi::c_void);
    fn i_human_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = i_human_init();
        assert!(!handle.is_null());

        let result = i_human_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libi_human = "libi_human"

function init()
    handle = ccall((:i_human_init, libi_human), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:i_human_process, libi_human), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:i_human_free, libi_human), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/i_human.h
   ```
